...
from tg_bot import TRIGGERS as trg, OWNER_ID as OWNER, pbot as anibot, DOWN_PATH

BOT_NAME = "@AsukaRobot"
LOG_CHANNEL_ID = "-1001644211791"

from tg_bot.utils.db import get_collection
from tg_bot.utils.helper import (
...
from tg_bot.utils.data_parser import (
...
async def start_(client: anibot, message: Message, mdata: dict):
    ...
    if gid==user:
        if not (user in OWNER) and not (await USERS.find_one({"id": user})):
            ...
            await clog("Asuka", f"New User started bot\n\n[{usertitle}](tg://user?id={user})\nID: `{user}`", "NEW_USER")
...
async def feed_(client: anibot, message: Message, mdata: dict):
    ...
    await client.send_message(mdata['chat']['id'], f"For issues or queries please contact @{owner} or join @AsukaSuppport")
